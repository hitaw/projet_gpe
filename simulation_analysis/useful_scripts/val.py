...
eqasim_train = eqasim_pt[eqasim_pt.transit_mode != "bus"]
# tram_to_keep = ["T4", "T9", "T11", "T12", "T13", "T14"]
tram_to_keep = []
codes = routes[routes.route_short_name.isin(tram_to_keep)]
codes = codes[codes.route_type==0]

eqasim_train = eqasim_train[(eqasim_train.transit_mode != "tram") | eqasim_train.transit_line_id.isin(codes.route_id)]
...
fig = px.histogram(melted_data, x='name', y='Value', color='Metric', barmode='group', title='Comparaison entre la référence et la simulation')
fig.show()
...
# merge_data = merge_data[merge_data.Percentage_Difference <0]
# print(len(merge_data))

fig_diff = px.bar(merge_data, x='name', y='Percentage_Difference',
                  title='Différence en pourcentage entre count et NB_VALD',
                  labels={'Percentage_Difference': 'Différence (%)'})
...
fig = px.histogram(melted_data, x='route_short_name', y='Value', color='Metric', barmode='group', title='Comparaison entre la référence et la simulation')
fig.show()
...
fig_diff = px.bar(diff_data, x='route_short_name', y='Différence (%)', title='Différence en pourcentage entre la Référence et la Simulation')
fig_diff.show()
